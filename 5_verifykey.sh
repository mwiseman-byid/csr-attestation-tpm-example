...
# Extract TPMT_PUBLIC from TPM2B_PUBLIC by removing first 2 octets
dd bs=2 skip=1 if=$acadir/key1.pub of=$acadir/key1.tpmt_public 2> /dev/null
# sed is needed to remove the leading characters added by openssl dgst
openssl dgst -sha256 $acadir/key1.tpmt_public | sed 's/.*= //' > $acadir/key1.local-name-hash
# Prepend name hashalg to be name
# Hard coding sha256 for now. Should read this from key1.pub output
